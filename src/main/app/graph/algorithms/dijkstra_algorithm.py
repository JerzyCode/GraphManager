...
def dijkstra_algorithm(graph, start, drawer):
    ...
    for count in range(len(graph.V)):
        min_dist_vertex = minimum_distance_vertex(graph, distance, done_vertexes, start)
        done_vertexes[min_dist_vertex] = True
        for_drawer.append([min_dist_vertex, distance[min_dist_vertex], 'set_vertex'])
        print(min_dist_vertex)
        for neigh in min_dist_vertex.neighbors:
            edge = neigh.find_edge(min_dist_vertex, isinstance(graph, DirectedGraph))
    ...
    graphics_dijkstra(drawer, for_drawer)


def _relaxation(distance, edge, edge_weight, prev):
    if distance[edge.vertex2] > distance[edge.vertex1] + edge_weight:
        distance[edge.vertex2] = distance[edge.vertex1] + edge_weight
        prev[edge.vertex2] = edge.vertex1


def minimum_distance_vertex(graph, distance, done_vertexes, start):
    min_distance = 1e7
    min_vertex = None
    for vertex in graph.V:
...
def graphics_dijkstra(drawer, for_drawer):
    delay = 0
    for element in for_drawer:
        ...
        if isinstance(element[0], Edge):
            ...
            delay += 500
    print('is done coloring dijkstra')
